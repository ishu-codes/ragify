...
class WorkspaceRepository:
    ...
    async def create_new_workspace(user_id: str):
        ...
        return await WorkspaceRepository.get_workspace_by_id(result.inserted_id)
    ...
    async def update_workspace(workspace_id: str, updates: dict):
        ...
        return await WorkspaceRepository.get_workspace_by_id(workspace_id)
...
class SessionRepository:
    ...
    async def get_sessions_by_workspace_id(workspace_id: str):
        cursor = (
            db["sessions"].find({"workspace_id": workspace_id}).sort("created_at", -1)
        )
        return await cursor.to_list(length=None)
    ...
    async def create_session(workspace_id: str):
        ...
        return await SessionRepository.get_session_by_id(result.inserted_id)
    ...
    async def update_session(session_id: str, updates: dict):
        ...
        return await SessionRepository.get_session_by_id(session_id)
...
class UploadRepository:
    ...
    async def create_upload_status(workspace_id: str, user_id: str, files: list[dict]):
        ...
        return await UploadRepository.get_upload_status_by_id(str(result.inserted_id))
    ...
    async def update_upload_status(status_id: str, updates: dict):
        ...
        return await UploadRepository.get_upload_status_by_id(status_id)
