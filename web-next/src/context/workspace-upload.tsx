...
const WorkspaceUploadContext =
  createContext<WorkspaceUploadContextValue | null>(null);
...
export function WorkspaceUploadProvider({
  ...
  const [activeUploadStatusId, setActiveUploadStatusId] = useState<
    string | null
  >(() => {
  ...
  const uploadStatusQuery = useQuery({
    ...
    queryFn: () => {
      ...
      const statusId = activeUploadStatusId;
      if (!token || !statusId) {
        throw new Error("Upload status is unavailable");
      }
      return workspaceApi.uploadStatus(token, workspaceId, statusId);
    },
    enabled: Boolean(
      session?.accessToken && workspaceId && activeUploadStatusId,
    ),
...
export function useWorkspaceUpload() {
  ...
  if (!context) {
    throw new Error(
      "useWorkspaceUpload must be used within WorkspaceUploadProvider",
    );
  }
