...
export default function WorkspaceOverviewPage() {
  ...
  const stats = [
    {
      label: "Materials",
      value: workspaceQuery.data?.materials.length ?? 0,
      icon: FilesIcon,
    },
    {
    ...
    {
      label: "Tags",
      value: workspaceQuery.data?.tags.length ?? 0,
      icon: TagsIcon,
    },
  ];
            ...
            <div className="space-y-3">
              ...
              <div className="flex flex-wrap gap-1.5">
                {workspaceQuery.data?.tags.length ? (
                  ...
                  <Badge
                    variant="outline"
                    className="rounded-full text-[11px] font-medium text-muted-foreground"
                  >
                    No tags
                  </Badge>
                )}
          ...
          <div className="mt-5 space-y-3">
            {chatSession.messages.length === 0 ? (
              <div className="border border-dashed border-border bg-muted/20 p-6 text-center text-sm text-muted-foreground">
                No local conversation yet. Start in chat after you upload some
                materials.
              </div>
            ) : (
              chatSession.messages
                ...
                .map((message) => (
                  ...
                  >
                    ...
                    <p className="line-clamp-4 text-sm leading-6 break-words whitespace-pre-wrap">
                      {message.content}
                    </p>
                  </div>
                ))
